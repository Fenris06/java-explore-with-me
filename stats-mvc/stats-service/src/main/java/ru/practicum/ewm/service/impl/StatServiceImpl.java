...
public class StatServiceImpl implements StatService {
    ...
    public List<HitDTO> getStats(LocalDateTime start, LocalDateTime end, List<String> uris, Boolean unique) {
        ...
        if (uris.isEmpty()) {
            if (!unique) {
                log.info("Exception in getStatAll with start {} and end {} unique = false", start, end);
                return repository.getStatAll(start, end)
                        .stream()
                        .map(Mapper::hitToDTO)
                        .collect(Collectors.toList());
            }
            log.info("Exception in getStatAllDistinct with start {} and end {} unique = true", start, end);
        ...
        if (!unique) {
            log.info("Exception in getStat with start {} and end {} and List<uris> {} unique = false", start, end, uris);
            return repository.getStat(start, end, uris)
                    .stream()
                    .map(Mapper::hitToDTO)
                    .collect(Collectors.toList());
        }
        log.info("Exception in getStatDistinct with start {} and end {} and List<uris> {} unique = true", start, end, uris);
